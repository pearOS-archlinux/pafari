...
GActionGroup *ephy_window_get_action_group (EphyWindow *window, const char *prefix);
#define EPHY_WINDOW(obj) (G_TYPE_CHECK_INSTANCE_CAST ((obj), ephy_window_get_type (), EphyWindow))
#define EPHY_IS_WINDOW(obj) (G_TYPE_CHECK_INSTANCE_TYPE ((obj), ephy_window_get_type ()))
 ...
 * @short_description: Pafari custom #WebkitWebView
 *
...
static void
privacy_report_message_received_cb (WebKitUserContentManager *ucm,
                                    JSCValue                 *message,
                                    EphyWebView              *view)
{
  GtkRoot *root;
  ...
  if (!EPHY_IS_WINDOW (root_widget))
    return;

  window = EPHY_WINDOW (root_widget);
  action_group = ephy_window_get_action_group (window, "win");
  if (!action_group)
    return;

  action = g_action_map_lookup_action (G_ACTION_MAP (action_group), "privacy-report");
  if (action)
                                        ...
                                        EphyWebViewMessageHandlerScope  scope)
{
   ...
   * script world so they are accessible to Pafari's internal web content. Use
   * main world message handlers only for internal Pafari pages. They should
  ...
  switch (handler) {
    ...
    case EPHY_WEB_VIEW_ABOUT_APPS_MESSAGE_HANDLER:
      ...
      g_signal_connect_object (ucm, "script-message-received::aboutApps",
                               ...
                               view, 0);
      break;
    case EPHY_WEB_VIEW_PRIVACY_REPORT_MESSAGE_HANDLER:
      webkit_user_content_manager_register_script_message_handler (ucm, "privacyReport", NULL);
      g_signal_connect_object (ucm, "script-message-received::privacyReport",
                               G_CALLBACK (privacy_report_message_received_cb),
                               view, 0);
      break;
  }
...
unregister_message_handlers (EphyWebView *view)
{
  ...
  if (view->message_handlers_to_unregister & EPHY_WEB_VIEW_PRIVACY_REPORT_MESSAGE_HANDLER) {
    webkit_user_content_manager_unregister_script_message_handler (ucm, "privacyReport", NULL);
    g_signal_handlers_disconnect_by_func (ucm, privacy_report_message_received_cb, view);
  }
                         ...
                         const char  **style)
{
  ...
  *message_title = g_strdup (_("Oops! There may be a problem"));
  ...
  first_paragraph = g_strdup_printf (_("The page %s may have caused Pafari to "
                                       "close unexpectedly."),
                                     formatted_uri);
                           ...
                           WebKitCredential            *credential)
{
  ...
  ephy_password_manager_save (password_manager,
                              ...
                              "org.gnome.Pafari.HTTPAuthCredentials.Username",
                              "org.gnome.Pafari.HTTPAuthCredentials.Password",
                              TRUE);
}
