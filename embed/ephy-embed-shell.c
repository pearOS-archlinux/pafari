...
ephy_resource_request_cb (WebKitURISchemeRequest *request)
{
  ...
  error = g_error_new (WEBKIT_NETWORK_ERROR, WEBKIT_NETWORK_ERROR_FAILED,
                       _("URI %s not authorized to access Pafari resource %s"),
                       uri, path);
  webkit_uri_scheme_request_finish_error (request, error);
}
                     ...
                     WebKitDownload *download)
{
   ...
   * explicitly by Pafari are marked with ephy-download-set GObject data.
   */
                               ...
                               WebKitUserContentManager *ucm)
{
   ...
   * implement internal Pafari pages, register it using
