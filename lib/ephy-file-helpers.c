 ...
 * ephy_file_tmp_dir:
 *
 * Returns the name of the temp dir for the running Pafari instance.
 *
 * Returns: the name of the temp dir, this string belongs to Pafari.
 **/
const char *
ephy_file_tmp_dir (void)
{
  if (!tmp_dir) {
    ...
    partial_name = g_strconcat ("pafari-", g_get_user_name (),
                                "-XXXXXX", NULL);
 ...
 * ephy_profile_dir:
 *
 * Gets Pafari's configuration directory, usually .local/share/pafari
 * under user's homedir.
 *
 * Returns: the full path to Pafari's configuration directory
 **/
const char *
 ...
 * ephy_config_dir:
 *
 * Gets Pafari's configuration directory, usually .config/pafari
 * under user's homedir.
 *
 * Returns: the full path to Pafari's configuration directory
 **/
 ...
 * ephy_cache_dir:
 *
 * Gets Pafari's cache directory, usually .cache/pafari
 * under user's homedir.
 *
 * Returns: the full path to Pafari's cache directory
 **/
const char *
ephy_cache_dir (void)
{
...
ephy_default_profile_dir (void)
{
  return profile_dir_type == EPHY_PROFILE_DIR_TEST ?
         ...
         g_build_filename (g_get_user_data_dir (), "pafari", NULL);
}
...
ephy_default_cache_dir (void)
{
  return profile_dir_type == EPHY_PROFILE_DIR_TEST ?
         ...
         g_build_filename (g_get_user_cache_dir (), "pafari", NULL);
}
...
ephy_default_config_dir (void)
{
  return profile_dir_type == EPHY_PROFILE_DIR_TEST ?
         ...
         g_build_filename (g_get_user_config_dir (), "pafari", NULL);
}

/**
 * ephy_file_helpers_init:
 * @profile_dir: directory to use as Pafari's profile
 * @flags: the %EphyFileHelpersFlags for this session
 * @error: an optional #GError
 *
 * Initializes Pafari file helper functions, sets @profile_dir as Pafari's
