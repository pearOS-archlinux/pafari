...
sync_chromes_visibility (EphyHeaderBar *header_bar)
{
  ...
  ephy_action_bar_start_set_show_bookmarks_button (header_bar->action_bar_start,
                                                   chrome & EPHY_WINDOW_CHROME_BOOKMARKS);
  if (header_bar->action_bar_end) {
    GtkWidget *page_menu_button = ephy_action_bar_end_get_page_menu_button (header_bar->action_bar_end);
    if (page_menu_button)
      gtk_widget_set_visible (page_menu_button, chrome & EPHY_WINDOW_CHROME_MENU);
  }
}
...
hide_timeout_cb (EphyHeaderBar *header_bar)
{
  ...
  page_menu_button = ephy_action_bar_end_get_page_menu_button (header_bar->action_bar_end);
  if (page_menu_button)
    ephy_page_menu_button_popdown (EPHY_PAGE_MENU_BUTTON (page_menu_button));
...
ephy_header_bar_constructed (GObject *object)
{
  ...
  /* Page Menu - now in action_bar_end */
  header_bar->page_menu_button = NULL;
...
ephy_header_bar_get_page_menu_button (EphyHeaderBar *header_bar)
{
  if (!header_bar->action_bar_end)
    return NULL;
  return ephy_action_bar_end_get_page_menu_button (header_bar->action_bar_end);
}
  ...
  switch (adaptive_mode) {
    case EPHY_ADAPTIVE_MODE_NORMAL:
      ...
      gtk_widget_set_visible (GTK_WIDGET (header_bar->action_bar_end), TRUE);
      {
        GtkWidget *page_menu_button = ephy_action_bar_end_get_page_menu_button (header_bar->action_bar_end);
        if (page_menu_button)
          gtk_widget_set_visible (page_menu_button, TRUE);
      }
    ...
    case EPHY_ADAPTIVE_MODE_NARROW:
      ...
      gtk_widget_set_visible (GTK_WIDGET (header_bar->action_bar_end), FALSE);
      {
        GtkWidget *page_menu_button = ephy_action_bar_end_get_page_menu_button (header_bar->action_bar_end);
        if (page_menu_button)
          gtk_widget_set_visible (page_menu_button, FALSE);
      }
