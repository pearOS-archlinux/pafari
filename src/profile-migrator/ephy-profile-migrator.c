...
migrate_pre_flatpak_webapps (void)
{
  ...
  while (info) {
    ...
    if (!g_str_has_prefix (name, "org.gnome.Pafari.WebApp-"))
      goto next;

    /* the directory is either org.gnome.Pafari.WebApp-name-checksum or
     * org.gnome.Pafari.WebApp-checksum but unfortunately name can include a
     * hyphen
     */
    {
       ...
       * org.gnome.Pafari.WebApp-<normalized_name>-<checksum> whereas the
...
migrate_nothing (void)
{
   ...
   * that support particularly ancient versions of Pafari.
   *
