...
static const GDBusErrorEntry ephy_webapp_provider_error_entries[] = {
  { EPHY_WEBAPP_PROVIDER_ERROR_FAILED, "org.gnome.Pafari.WebAppProvider.Error.Failed" },
  { EPHY_WEBAPP_PROVIDER_ERROR_INVALID_ARGS, "org.gnome.Pafari.WebAppProvider.Error.InvalidArgs" },
  { EPHY_WEBAPP_PROVIDER_ERROR_NOT_INSTALLED, "org.gnome.Pafari.WebAppProvider.Error.NotInstalled" },
};
                ...
                EphyWebAppProviderService *self)
{
   ...
   * since Pafari is not the focused application). We use the same code path
...
ephy_web_app_provider_service_new (void)
{
   ...
   */
  g_autofree gchar *app_id = g_strconcat ("org.gnome.Pafari.WebAppProvider", NULL);
