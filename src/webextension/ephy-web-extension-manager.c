...
/* We add Pafari to the UA so proactive extensions can rely on it always. */
#define FIREFOX_OVERRIDE "Mozilla/5.0 (X11; Linux x86_64; rv:101.0) Gecko/20100101 Firefox/101.0 Pafari/" EPHY_VERSION

  /* FIXME: These names are post translation. */

  /* Bitwarden has Safari specific hacks that cannot work on Pafari such as calling out to their host mac app. */
                                    ...
                                    gpointer           user_data)
{
  ...
  g_warning ("%s(): '%s' not implemented by Pafari!", __FUNCTION__, name);
...
ephy_web_extensions_manager_create_web_extensions_webview (EphyWebExtension *web_extension)
{
  ...
  if (custom_user_agent)
    ...
    webkit_settings_set_user_agent_with_application_details (settings, "Pafari", EPHY_VERSION);
