 ...
 * @see_also: #GtkEntry
 *
 * #EphyLocationEntry implements the location bar in the main Pafari window.
 */
 ...
 * Pafari does not ever change it, we are just checking system default proxy.
 */
static void
...
ephy_location_entry_update_url_button_style (EphyLocationEntry *self)
{
  ...
  if (text && strlen (text) > 0) {
    ...
    /* Scheme is hidden, and www. prefix if present */
    start_hidden = pango_attr_shape_new (&empty_rect, &empty_rect);
    start_hidden->start_index = offset;
    {
      const char *host_pos = strstr (text, host);
      if (host_pos) {
        /* If host starts with www., hide the www. part too */
        if (g_str_has_prefix (host, "www.")) {
          start_hidden->end_index = host_pos - text + 4; /* Skip "www." */
        } else {
          start_hidden->end_index = host_pos - text;
        }
      } else {
        start_hidden->end_index = sub_string - text;
      }
    }
