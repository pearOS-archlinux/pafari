...
For tips for hacking on Pafari, see the HACKING.md file.
...
Check the About dialog if you're not sure what Pafari or WebKitGTK versions
you have.
...
or Pafari features (e.g. Firefox Sync, adblocker, password manager, web apps)
should be reported on Pafari's GitLab issue tracker.

Don't worry if you accidentally submit your report in the wrong place. This
happens all the time, since it's sometimes difficult to guess whether Pafari
or WebKit is responsible for a bug. If we suspect an issue reported on the
Pafari issue tracker is actually a WebKit bug, we will close it and ask you to
report the issue on WebKit Bugzilla instead.

# Crashes

If Pafari crashed, then we really need a backtrace taken in gdb with `bt full`
